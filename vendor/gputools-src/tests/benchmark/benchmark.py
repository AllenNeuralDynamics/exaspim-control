...
def bench(description, dshape, dtype, func_cpu, func_gpu, func_gpu_notransfer=None, niter=2):
    ...
    print(
        ...
        % (
            description,
            dshape,
            type_name_dict[dtype],
            1000 * t_cpu,
            1000 * t_gpu,
            "%d ms" % (1000 * t_gpu_notransfer) if t_gpu_notransfer is not None else "-",
        )
    )
...
if __name__ == "__main__":
    ...
    bench(
        ...
        cut(dshape),
        np.uint8,
        lambda x: ndimage.geometric_transform(x, lambda c: c, output_shape=x.shape, order=1, prefilter=False),
