...
def _test_single(filt1, filt2, dshape, size, cval=0.0, dtype=np.float32, strides=(1, 1), skip_assert=False):
    ...
    print(
        (
            "shape: %s \tsize: %s\t cval: %.2f\t dtype: %s\t stride: %s \tdifference: %s"
            % (dshape, size, cval, dtype, strides, np.amax(np.abs(1.0 * out1 - out2)))
        )
    )
    if not skip_assert:
...
def _test_some(filt1, filt2, cval=0.0):
...
def test_all():
    ...
    _test_some(uniform_filter, spf.uniform_filter, cval=0.0)
...
if __name__ == "__main__":
    ...
    x, a, b = _test_single(
        uniform_filter, spf.uniform_filter, (4, 4), 2, strides=(1, 1), dtype=np.uint8, cval=0, skip_assert=True
    )
