...
def create_psf(sig=(0.1, 0.1), xy_angle=0.0, N=10):
    ...
    h = np.exp(-reduce(np.add, [_X**2 / _s**2 / 2.0 for _X, _s in zip([Y2, X2], sig)]))
    h *= 1.0 / np.sum(h)
    return h
    ...
    return np.stack(
        [
            np.stack(
                [
                    create_psf(
                        sig=(0.01 + 0.1 * np.sqrt(_x**2 + _y**2), 0.01 + 0.4 * np.sqrt(_x**2 + _y**2)),
                        N=N,
                        xy_angle=-1.0 * np.pi + np.arctan2(_y, _x),
                    )
...
def psf_grid_const(Gx, Gy, N=21, sx=0.01, sy=0.01):
    return np.stack(
        [
            np.stack([create_psf(w=0, N=N, sx=sx, sy=sy) for _y in np.linspace(-1, 1, Gy)])
            for _x in np.linspace(-1, 1, Gx)
        ]
    )


def create_psf3(sig=(0.3, 0.3, 0.3), N=10, xy_angle=0.0):
    x = np.linspace(-1, 1, N + 1)[:-1]
    Z, Y, X = np.meshgrid(x, x, x, indexing="ij")
    X2 = np.cos(xy_angle) * X - np.sin(xy_angle) * Y
    Y2 = np.cos(xy_angle) * Y + np.sin(xy_angle) * X
    h = np.exp(-reduce(np.add, [_X**2 / _s**2 / 2.0 for _X, _s in zip([Z, Y2, X2], sig)]))
    h *= 1.0 / np.sum(h)
    return h
...
def psf_grid_const3(Gx, Gy, Gz, N=21, sig=(0.01, 0.01, 0.01)):
    return np.stack(
        [np.stack([np.stack([create_psf3(N=N, sig=sig) for _ in range(Gx)]) for _ in range(Gy)]) for _ in range(Gz)]
    )
        ...
        [
            np.stack(
                [
                    np.stack(
                        [
                            create_psf3(N=N, sig=(0.1 + 0.4 * x**2, 0.001 + 0.2 * x**2, 0.001 + 0.2 * x**2))
...
def create_prolate_psf3(N=21, w=(0, 1, 0), s1=0.4, s2=0.1):
    x = np.linspace(-1, 1, N + 1)[:-1]
    Xs = np.array(np.meshgrid(x, x, x, indexing="ij"))

    w = np.array(w)
    w = w * 1.0 / np.sqrt(np.sum(w**2))

    r1 = np.dot(Xs.T, w)
    r2 = np.sqrt(np.sum(np.cross(Xs.T, w) ** 2, axis=-1))

    h = np.exp(-(r1**2) / 2.0 / s1**2 - r2**2 / 2.0 / s2**2)
    h *= 1.0 / np.sum(h)
    return h


def psf_grid_motion3(Gx=2, Gy=2, Gz=2, N=21, s1=0.4, s2=0.1, omega=(0, 1, 0)):
    return np.stack(
        [
            np.stack(
                [
                    np.stack(
                        [
                            create_prolate_psf3(w=np.cross(omega, [_x, _y, _z]), N=N, s1=s1, s2=s2)
                            for _x in np.linspace(-1, 1, Gx)
                        ]
                    )
                    for _y in np.linspace(-1, 1, Gy)
...
def test_single_z():
    ...
    hs[:, Nx // Ng // 2 :: Nx // Ng, Nx // Ng // 2 :: Nx // Ng] = 1.0
...
def test_identity2():
    ...
    h = np.zeros_like(im)
    h[Ny // Ng // 2 :: Ny // Ng, Nx // Ng // 2 :: Nx // Ng] = 1.0
