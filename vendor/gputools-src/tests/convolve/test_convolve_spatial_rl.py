...
def create_psf(sig=(0.1, 0.1), xy_angle=0.0, N=10, center=(0, 0)):
    ...
    h = np.exp(-reduce(np.add, [(_X - _c) ** 2 / _s**2 / 2.0 for _X, _s, _c in zip([Y2, X2], sig, center)]))
    h *= 1.0 / np.sum(h)
    return h
            ...
            np.stack(
                [
                    create_psf(
                        ...
                        N=N,
                        center=(0.4 * (_x**2 - 1.0), 0.4 * (_y**2 - 1.0)),
                    )
                    for _y in np.linspace(-1, 1, Gy)
                ]
            )
...
def psf_grid_const(Gx, Gy, N=21, sx=0.01, sy=0.01, center=(0, 0)):
    return np.stack(
        [
            np.stack([create_psf(sig=(sy, sx), N=N, center=center) for _y in np.linspace(-1, 1, Gy)])
            for _x in np.linspace(-1, 1, Gx)
        ]
    )
...
if __name__ == "__main__":
    ...
    hs0 = psf_grid_const(Gx, Gy, sx=0.3, sy=0.3, center=(0.2, 0.2))
