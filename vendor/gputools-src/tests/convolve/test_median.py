...
def _test_single(dshape, size, cval=0.0, dtype=np.float32, skip_assert=False):
    ...
    print(
        (
            "shape: %s \tsize: %s\tcval: %.2f\tdtype: %s\tdifference: %.2f"
            % (dshape, size, cval, dtype, np.amax(np.abs(out1 - out2)))
        )
    )
    if not skip_assert:
