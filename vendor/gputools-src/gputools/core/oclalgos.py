...
if __name__ == "__main__":
    ...
    k = OCLElementWiseKernel("float *a_g, float b", "a_g[i] = b + a_g[i]", "add")

    k2 = OCLElementWiseKernel("cfloat_t *a_g, cfloat_t b", "a_g[i] = cfloat_add(b,a_g[i])", "add")

    r = OCLReductionKernel(
        np.complex64,
        neutral="0",
        reduce_expr="a+b",
        map_expr="cfloat_mul(x[i],y[i])",
        arguments="__global cfloat_t *x, __global cfloat_t *y",
    )
