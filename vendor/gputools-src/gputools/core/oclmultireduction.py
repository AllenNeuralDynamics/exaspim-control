...
from pyopencl.tools import context_dependent_memoize, dtype_to_ctype, KernelTemplateBase, _process_code_for_macro
import numpy as np
...
def get_reduction_kernel(
    ...
    from pyopencl.tools import parse_arg_list, get_arg_list_scalar_arg_dtypes, get_arg_offset_adjuster_code, VectorArg
    ...
    if stage == 2 and arguments is not None:
        ...
        arguments = [VectorArg(dtype_out, "pyopencl_reduction_inp_%i" % i) for i in range(len(map_exprs))] + arguments
...
class OCLMultiReductionKernel:
    ...
    def __init__(
        ...
        self.stage_2_inf = get_reduction_kernel(
            2,
            ctx,
            dtype_out,
            neutral,
        ...
        assert any(isinstance(arg_tp, VectorArg) for arg_tp in self.stage_1_inf.arg_types), (
            "ReductionKernel can only be used with functions that have at least one vector argument"
        )
    ...
    def __call__(self, *args, **kwargs):
        ...
        while True:
            ...
            if group_count == 1:
                results = [
                    empty(use_queue, (), self.dtype_out, allocator=repr_vec.allocator) if out is None else out
                    for out in outs
                ]
            else:
                results = [
                    empty(use_queue, (group_count,), self.dtype_out, allocator=repr_vec.allocator) for out in outs
                ]
...
if __name__ == "__main__":
    ...
    k1 = OCLReductionKernel(np.float32, neutral="0", reduce_expr="a+b", map_expr="x[i]", arguments="__global float *x")
    ...
    b = OCLArray.from_array(2.0 * np.ones((N, N), np.float32))
