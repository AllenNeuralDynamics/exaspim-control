...
def assert_bufs_type(mytype, *bufs):
    if not all([b.dtype.type == mytype for b in bufs]):
        raise TypeError(
            "all data type of buffer(s) should be %s! but are %s" % (mytype, str([b.dtype.type for b in bufs]))
        )
...
def _wrap_OCLArray(cls):
    ...
    def copy_image(self, img, **kwargs):
        ...
        return cl.enqueue_copy(
            queue, self.data, img, offset=0, origin=(0,) * len(img.shape), region=img.shape, **kwargs
        )
...
def _wrap_OCLImage(cls):
    ...
    def from_array(cls, arr, *args, **kwargs):
        ...
        if arr.dtype.type == np.complex64:
            ...
            res = cl.image_from_array(ctx, prepare(arr), num_channels=num_channels, *args, **kwargs)
    ...
    def copy_buffer(self, buf, **kwargs):
        ...
        return cl.enqueue_copy(
            queue, self, buf.data, offset=0, origin=(0,) * len(self.imshape()), region=self.imshape(), **kwargs
        )
    ...
    def copy_image_resampled(self, img, **kwargs):
        ...
        OCLArray._resample_prog.run_kernel(kern_str, self.imshape(), None, img, self)
    ...
    def write_array(self, arr):
        ...
        if dshape != imshape[::-1]:
            ...
            cl.enqueue_copy(queue, self, arr.copy(), origin=(0,) * ndim, region=imshape)
    ...
    def copy_buffer(self, buf):
        ...
        cl.enqueue_copy(queue, self, buf.data, offset=0, origin=(0,) * ndim, region=imshape)
