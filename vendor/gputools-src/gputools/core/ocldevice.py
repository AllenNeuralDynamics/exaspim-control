...
class OCLDevice:
    ...
    def device_priority(cls, device_with_type_tuple):
        ...
        return (
            device_type is pyopencl.device_type.GPU,
            device.get_info(pyopencl.device_info.GLOBAL_MEM_SIZE),
        )

    def init_cl(self, id_platform=-1, id_device=-1, use_gpu=True, print_info=False, context_properties=None):
        ...
        try:
            ...
            logger.warning(
                ...
                "...choosing the best from the rest" % (id_platform, id_device, device_type)
            )
            # get the best available device
            device, _ = max([(d, t) for (_ip, _id, t), d in all_platforms_devs.items()], key=OCLDevice.device_priority)

        if device is None:
            raise Exception("Failed to find a valid device")

        self.context = pyopencl.Context(devices=[device], properties=context_properties)
        ...
        self.imageformats = pyopencl.get_supported_image_formats(
            self.context, pyopencl.mem_flags.READ_WRITE, pyopencl.mem_object_type.IMAGE3D
        )
    ...
    def get_all_info(self):
        ...
        for p in platforms:
            s += "platform: \t%s\n" % p.name
            printNames = [["CPU", pyopencl.device_type.CPU], ["GPU", pyopencl.device_type.GPU]]
