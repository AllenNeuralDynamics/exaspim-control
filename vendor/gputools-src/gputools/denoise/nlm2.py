...
def nlm2(data, sigma, size_filter=2, size_search=3):
    ...
    prog = OCLProgram(abspath("kernels/nlm2.cl"), build_options="-D FS=%i -D BS=%i" % (size_filter, size_search))
    ...
    for dx in range(size_search + 1):
        for dy in range(-size_search, size_search + 1):
            prog.run_kernel("dist", img.shape, None, img, tmpImg, np.int32(dx), np.int32(dy))
            prog.run_kernel("convolve", img.shape, None, tmpImg, tmpImg2, np.int32(1))
            prog.run_kernel("convolve", img.shape, None, tmpImg2, distImg, np.int32(2))
