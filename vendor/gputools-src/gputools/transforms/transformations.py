...
def affine(data, mat=np.identity(4), output_shape=None, mode="constant", interpolation="linear", res_g=None):
    ...
    prog = OCLProgram(
        abspath("kernels/affine.cl"), build_options=interpolation_defines[interpolation] + mode_defines[mode]
    )

    prog.run_kernel("affine3", output_shape[::-1], None, d_im, res_g.data, mat_inv_g.data)
...
def rotate(data, axis=(1.0, 0, 0), angle=0.0, center=None, mode="constant", interpolation="linear"):
    """
    ...
    cx, cy, cz = center
    m = np.dot(mat4_translate(cx, cy, cz), np.dot(mat4_rotate(angle, *axis), mat4_translate(-cx, -cy, -cz)))
    m = np.linalg.inv(m)
    return affine(data, m, mode=mode, interpolation=interpolation)


def map_coordinates(data, coordinates, interpolation="linear", mode="constant"):
    """
    ...
    prog = OCLProgram(
        abspath("kernels/map_coordinates.cl"),
        build_options=interpolation_defines[interpolation] + mode_defines[mode] + dtype_defines,
    )

    kernel = "map_coordinates{ndim}".format(ndim=data.ndim)

    prog.run_kernel(kernel, (coordinates.shape[-1],), None, d_im, res_g.data, coordinates_g.data)

    return res_g.get()


def geometric_transform(data, mapping="c0,c1", output_shape=None, mode="constant", interpolation="linear"):
    """
    ...
    prog = OCLProgram(
        src_str=rendered,
        build_options=interpolation_defines[interpolation] + mode_defines[mode] + dtype_defines + image_read_defines,
    )
...
if __name__ == "__main__":
    ...
    d[20:-20, 20:-20, 20:-20] = 1.0

    # res = translate(d, x = 10, y = 5, z= -10 )
    res = rotate(d, center=(100, 100, 100), angle=0.5)
