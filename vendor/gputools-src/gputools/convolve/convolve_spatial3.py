...
import sys
import numpy as np
from gputools import fft_plan, OCLArray, OCLImage, fft, get_device, OCLProgram, pad_to_shape, tile_iterator
from itertools import product
from gputools.utils.utils import _is_power2, next_power_of_2
from ._abspath import abspath


def convolve_spatial3(
    im, psfs, mode="constant", grid_dim=None, sub_blocks=None, pad_factor=2, plan=None, return_plan=False, verbose=False
):
    """
    ...
    if sub_blocks == None:
        return _convolve_spatial3(
            im, psfs, mode=mode, pad_factor=pad_factor, plan=plan, return_plan=return_plan, grid_dim=grid_dim
        )
    else:
        ...
        if grid_dim:
            ...
            plan = None
            for i, ((im_tile, im_s_src, im_s_dest), (hs_tile, hs_s_src, hs_s_dest)) in enumerate(
                zip(
                    tile_iterator(im, blocksize=N_sub, padsize=Npads, mode=mode, verbose=verbose),
                    tile_iterator(psfs, blocksize=N_sub, padsize=Npads, mode=mode, verbose=verbose),
                )
            ):
                if verbose:
                    print("convolve_spatial3 ... %s\t/ %s" % (i + 1, np.prod(sub_blocks)))

                res_tile, plan = _convolve_spatial3(
                    im_tile.copy(),
                    hs_tile.copy(),
                    mode=mode,
...
def _convolve_spatial3(im, hs, mode="constant", grid_dim=None, plan=None, return_plan=False, pad_factor=2):
    if im.ndim != 3:
        raise ValueError("wrong dimensions of input!")
    ...
    prog = OCLProgram(abspath("kernels/conv_spatial3.cl"), build_options=["-D", "ADDRESSMODE=%s" % mode_str[mode]])
    ...
    if grid_dim:
        ...
        for (k, _z0), (j, _y0), (i, _x0) in product(*[enumerate(X) for X in Xs]):
            prog.run_kernel(
                ...
                np.int32(i * np.prod(Npatchs) + j * Gs[2] * np.prod(Npatchs) + k * Gs[2] * Gs[1] * np.prod(Npatchs)),
            )
    ...
    prog.run_kernel("mult_inplace", (np.prod(Npatchs) * np.prod(Gs),), None, patches_g.data, h_g.data)
