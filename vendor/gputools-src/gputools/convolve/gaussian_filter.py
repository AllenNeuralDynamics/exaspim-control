...
def gaussian_filter(data, sigma=4.0, truncate=4.0, strides=1, normalize=True, res_g=None):
    """
...
def _gaussian_buf(d_g, sigma=(4.0, 4.0), res_g=None, normalize=True, strides=1, truncate=4.0):
    ...
    hs = tuple(np.exp(-0.5 / max(s, 1e-10) ** 2 * n**2) for s, n in zip(reversed(sigma), reversed(ns)))

    if normalize:
        hs = tuple(1.0 * h / np.sum(h) for h in hs)
