...
def convolve(data, h, res_g=None, sub_blocks=None, mode="constant"):
    ...
    if isinstance(data, OCLArray) and isinstance(h, OCLArray):
        ...
        if sub_blocks == (1,) * data.ndim and mode == "constant":
            ...
            for data_tile, data_s_src, data_s_dest in tile_iterator(data, blocksize=N_sub, padsize=Npads, mode=mode):
...
def _convolve3_old(data, h, dev=None):
    ...
    dtypes_options = {np.float32: "", np.uint16: "-D SHORTTYPE"}
    ...
    prog = OCLProgram(abspath("kernels/convolve3.cl"), build_options=dtypes_options[dtype])
    ...
    prog.run_kernel("convolve3d", img.shape, None, img, hbuf.data, res.data, *Ns)
...
if __name__ == "__main__":
    ...
    ind = [np.random.randint(0, n, int(np.prod(d.shape) ** (1.0 / d.ndim)) / 10) for n in d.shape]
    d[tuple(ind)] = 1.0
    h *= 1.0 / np.sum(h)
