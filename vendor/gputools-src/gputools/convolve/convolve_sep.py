...
def convolve_sep2(data, hx, hy, res_g=None, strides=1, sub_blocks=None):
    ...
    if isinstance(data, np.ndarray):
        ...
        if sub_blocks == (1, 1) or sub_blocks is None:
            ...
            for data_tile, data_s_src, data_s_dest in tile_iterator(
                data, blocksize=N_sub, padsize=Npads, mode="constant"
            ):
                res_tile = _convolve_sep2_numpy(data_tile.copy(), hx, hy, strides=strides)
...
def _convolve_sep2_gpu(data_g, hx_g, hy_g, strides, res_g=None):
    ...
    prog = OCLProgram(
        abspath("kernels/convolve_sep.cl"),
        build_options=["-D", "DTYPE=%s" % cl_buffer_datatype_dict[data_g.dtype.type]],
    )
    ...
    if isinstance(data, np.ndarray):
        ...
        if sub_blocks == (1, 1, 1) or sub_blocks is None:
            ...
            for i, (data_tile, data_s_src, data_s_dest) in enumerate(
                ...
                res_tile = _convolve_sep3_numpy(data_tile.copy(), hx, hy, hz, strides=strides)
                res[data_s_src] = res_tile[data_s_dest]
            return res
...
def convolve_sep_approx(data, h, Nsep=2):
    ...
    hs = separable_series(h + 1.0e-40, Nsep)
...
def test_3d():
    ...
    hx = 1.0 / 5 * np.ones(5)
    hy = 1.0 / 13 * np.ones(13)
    hz = 1.0 / 13 * np.ones(11)

    t = time()
    for _ in range(Niter):
        out = convolve_sep3(data, hx, hy, hz)
    print("time: %.3f ms" % (1000.0 * (time() - t) / Niter))
