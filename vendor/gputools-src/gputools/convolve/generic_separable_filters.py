...
def _generic_filter_gpu_2d(FUNC="fmax(res,val)", DEFAULT="-INFINITY"):
    def _filt(data_g, size=(3, 3), res_g=None, strides=(1, 1)):
        ...
        rendered = tpl.render(FSIZE_X=size[-1], FSIZE_Y=size[-2], FSIZE_Z=1, FUNC=FUNC, DEFAULT=DEFAULT, DTYPE=DTYPE)
        ...
        Ny, Nx = data_g.shape
        prog.run_kernel(
            "filter_2_x", out_shape_x[::-1], None, data_g.data, tmp_g.data, np.int32(Nx), np.int32(strides[1])
        )

        prog.run_kernel(
            "filter_2_y", out_shape_y[::-1], None, tmp_g.data, res_g.data, np.int32(Ny), np.int32(strides[0])
        )
...
def _generic_filter_gpu_3d(FUNC="fmax(res,val)", DEFAULT="-INFINITY"):
    def _filt(data_g, size=(3, 3, 3), res_g=None, strides=(1, 1, 1)):
        ...
        rendered = tpl.render(
            FSIZE_X=size[-1], FSIZE_Y=size[-2], FSIZE_Z=size[-3], FUNC=FUNC, DEFAULT=DEFAULT, DTYPE=DTYPE
        )

        prog = OCLProgram(src_str=rendered, build_options=["-cl-unsafe-math-optimizations"])
        ...
        prog.run_kernel(
            "filter_3_x", out_shape_x[::-1], None, data_g.data, tmp_g.data, np.int32(Nx), np.int32(strides[2])
        )
        prog.run_kernel(
            "filter_3_y", out_shape_y[::-1], None, tmp_g.data, tmp2_g.data, np.int32(Ny), np.int32(strides[1])
        )
        prog.run_kernel(
            "filter_3_z", out_shape_z[::-1], None, tmp2_g.data, res_g.data, np.int32(Nz), np.int32(strides[0])
        )
        return res_g
        ...
        if isinstance(data, np.ndarray):
            if sub_blocks is None or set(sub_blocks) == {1}:
                ...
                for i, (data_tile, data_s_src, data_s_dest) in enumerate(
                    tile_iterator(data, blocksize=N_sub, padsize=Npads, mode="constant")
                ):
                    res_tile = _filter_numpy(data_tile.copy(), size, strides=strides)
...
def uniform_filter(data, size=7, res_g=None, strides=1, sub_blocks=(1, 1, 1), cval=0, normalized=True):
    ...
    if normalized:
        if np.isscalar(size):
            ...
            norm = np.int32(np.prod(size)) ** (1.0 / len(size))
    ...
    if data.ndim == 2:
        _filt = make_filter(
            _generic_filter_gpu_2d(
                FUNC="res+(val*native_exp((float)(-(ht-%s)*(ht-%s)/2/%s/%s)))" % (size[0] // 2, size[0] // 2, s, s),
                DEFAULT=cval,
            )
        )
    elif data.ndim == 3:
