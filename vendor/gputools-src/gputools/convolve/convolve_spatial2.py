...
def convolve_spatial2(
    im, psfs, mode="constant", grid_dim=None, sub_blocks=None, pad_factor=2, plan=None, return_plan=False
):
    """
    ...
    if sub_blocks is None:
        ...
        logger.debug("N_sub: {}, Nblocks: {}, grid_dim_sub, {}, Npads, {}".format(N_sub, Nblocks, grid_dim_sub, Npads))

        if grid_dim:
            res = np.empty(im.shape, np.float32)
            plan = None
            for (im_tile, im_s_src, im_s_dest), (hs_tile, hs_s_src, hs_s_dest) in zip(
                tile_iterator(im, blocksize=N_sub, padsize=Npads, mode=mode),
                tile_iterator(psfs, blocksize=N_sub, padsize=Npads, mode=mode),
            ):
                res_tile, plan = _convolve_spatial2(
                    im_tile.copy(),
                    hs_tile.copy(),
                    mode=mode,
...
def _convolve_spatial2(im, hs, mode="constant", grid_dim=None, pad_factor=2, plan=None, return_plan=False):
    """
    ...
    prog = OCLProgram(abspath("kernels/conv_spatial2.cl"), build_options=["-D", "ADDRESSMODE=%s" % mode_str[mode]])
    ...
    prog.run_kernel("mult_inplace", (Npatch_x * Npatch_y * Gx * Gy,), None, patches_g.data, h_g.data)
