...
def tile_iterator(im, blocksize=(64, 64), padsize=(64, 64), mode="constant", verbose=False):
    ...
    if not (im.ndim == len(blocksize) == len(padsize)):
        raise ValueError(
            "im.ndim (%s) != len(blocksize) (%s) != len(padsize) (%s)" % (im.ndim, len(blocksize), len(padsize))
        )
    ...
    for i, index in enumerate(product(*[range(sg) for sg in subgrids])):
        ...
        s_output = tuple(
            [
                ...
                for b, pm, p, i, s in zip(blocksize, pad_mismatch, padsize, index, subgrids)
            ]
        )
...
if __name__ == "__main__":
    ...
    for n in [1, 2, 3]:
        ...
        for padded_tile, s_src, s_dest in tile_iterator(
            im, blocksize=(50,) * im.ndim, padsize=(64,) * im.ndim, mode="wrap"
        ):
            # reassemble the result at the correct position
