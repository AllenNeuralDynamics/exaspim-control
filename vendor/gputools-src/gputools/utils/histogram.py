...
def histogram(x, n_bins=256):
    ...
    local_size = min(get_device().get_info("MAX_WORK_GROUP_SIZE"), 2 ** int(np.log2(np.sqrt(len(x)))))
    ...
    prog = OCLProgram(
        abspath("kernels/histogram.cl"),
        build_options=["-D", "N_BINS=%s" % n_bins, "-D", "RED_SIZE=%s" % red_size, "-D", "DTYPE=%s" % DTYPE],
    )

    prog.run_kernel(
        "histogram_partial", (len(x),), (local_size,), x_g.data, part_hist_g.data, np.float32(x0), np.float32(x1)
    )

    prog.run_kernel("histogram_sum", (n_bins,), None, part_hist_g.data, hist_g.data)
