...
def pad_to_shape(d, dshape, mode="constant"):
    ...
    # then pad
    return np.pad(
        res, tuple((int(np.ceil(d / 2.0)), d - int(np.ceil(d / 2.0))) if d > 0 else (0, 0) for d in diff), mode=mode
    )
...
def get_cache_dir():
    ...
    return os.path.join(
        ...
        "pyopencl-compiler-cache-v2-py%s" % (".".join(str(i) for i in sys.version_info),),
    )
