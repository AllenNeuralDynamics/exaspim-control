...
class ChannelPlanWidget(QTabWidget):
    ...
    channelAdded = pyqtSignal(str)
    channelChanged = pyqtSignal()

    def __init__(self, instrument, properties: dict, unit: str = "um"):
        """
        :param instrument: voxel instrument model
        :param properties: allowed properties for devices (from config)
        ...
        self.possible_channels = instrument.channels
        self.channels = []
    ...
    def initialize_tables(self) -> None:
        ...
        Configuration is derived from the instrument model's property descriptors.
        """
        ...
        for channel in self.possible_channels:
            ...
            delegates = [
                QSpinItemDelegate(),
                QSpinItemDelegate(minimum=0, step=1),
                QTextItemDelegate(),
            ]
            ...
            for device_type, properties in self.properties.items():
                if device_type in self.possible_channels[channel]:
                    for device_name in self.possible_channels[channel][device_type]:
                        # Get device object directly from instrument model
                        ...
                        for prop in properties:
                            ...
                            # Get constraints from descriptor attributes
                            minimum = getattr(descriptor, "minimum", float("-inf"))
                            maximum = getattr(descriptor, "maximum", float("inf"))
                            step = getattr(descriptor, "step", 0.1)

                            # Determine delegate type based on descriptor characteristics
                            if hasattr(descriptor, "fget") and hasattr(descriptor.fget, "__self__"):
                                # Check if property has enum-like behavior (discrete values)
                                try:
                                    # Try to get current value to determine if it's a combo-type property
                                    getattr(device_object, prop)
                                    # For now, use spin delegate for numeric types
                                    if input_type in [int, float] or input_type == inspect._empty:
                                        delegates.append(
                                            QSpinItemDelegate(
                                                minimum=minimum,
                                                maximum=maximum,
                                    ...
                                    # Default to spin delegate for numeric properties
                                    delegates.append(QSpinItemDelegate(minimum=minimum, maximum=maximum, step=step))
                            else:
                                delegates.append(QSpinItemDelegate(minimum=minimum, maximum=maximum, step=step))

                            # Store value getter function from device object
                            setattr(
                                self,
                                column_name + "_value_function",
                                lambda obj=device_object, p=prop: getattr(obj, p),
                            )
                    ...
                    setattr(
                        self,
                        column_name + "_initial_value",
                        properties.get("initial_value", None),
                    )
                    columns.append(column_name)
    ...
    def apply_all(self) -> bool:
        """pyqtProperty for the state of apply all
    ...
    def tile_volumes(self) -> np.ndarray:
        """
        pyqtProperty of tile volumes in 2d numpy array
    ...
    def update_step_size(self, tile_index: list[int], row: int, channel: str) -> list[float, int]:
        ...
        if step_size != 0 and not isnan(step_size) and step_size not in [float("inf"), float("-inf")]:
            steps = round(volume_um / (step_size * self.micron))
