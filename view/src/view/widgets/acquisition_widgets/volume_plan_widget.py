...
from view.widgets.miscellaneous_widgets.q_start_stop_table_header import (
    QStartStopTableHeader,
)
...
class GridFromEdges(useq.GridFromEdges):
    ...
    @property
    def rows(self) -> int:
        """pyqtProperty that returns number of rows in configured scan"""
        dx, _ = self._step_size(self.fov_width, self.fov_height)
        return self._nrows(dx)

    @property
    def columns(self) -> int:
        """pyqtProperty that returns number of columns in configured scan"""
...
class VolumePlanWidget(QMainWindow):
    ...
    def __init__(
        self,
        limits: list[[float, float], [float, float], [float, float]] | None = None,
        fov_dimensions: list[float, float, float] | None = None,
        fov_position: list[float, float, float] | None = None,
        coordinate_plane: list[str, str, str] | None = None,
        unit: str = "um",
        ...
        if default_order not in valid_orders:
            msg = f"Invalid default order {default_order}. Must be one of {valid_orders}"
    ...
    def toggle_visibility(self, checked: bool, row: int, column: int) -> None:
        ...
        if self.apply_all and [row, column] == [
            0,
            0,
        ]:  # trigger update of all subsequent checkboxes
    ...
    def tile_table_changed(self, item: QTableWidgetItem) -> None:
        ...
        if col_title in titles:
            ...
            if self.apply_all and [row, column] == [
                0,
                0,
            ]:  # trigger update of all subsequent tiles
                for r in range(self.tile_table.rowCount()):
    ...
    def apply_all(self, value: bool) -> None:
        ...
        if value:
            self.blockSignals(True)  # emit pyqtSignal only once
    ...
    def _on_change(self) -> None:
        ...
        of tiles and emits pyqtSignal when done.
        """
