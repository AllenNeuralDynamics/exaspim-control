...
class BaseDeviceWidget(QMainWindow):
    ...
    def create_property_widgets(self, properties: dict, widget_group):
        ...
        for name, value in properties.items():
            ...
            if (dict not in type(value).__mro__ and list not in type(value).__mro__) or isinstance(
                arg_type, enum.EnumMeta
            ):
                ...
                boxes[name] = create_widget(
                    "V",
                    **self.create_property_widgets({f"{name}.{k}": v for k, v in value.items()}, name),
                )
            elif list in type(value).__mro__:  # deal with list like variables
                setattr(self, f"{name}_schema", Schema(create_list_schema(value)))
                boxes[name] = create_widget(
                    "H",
                    **self.create_property_widgets({f"{name}.{i}": v for i, v in enumerate(value)}, name),
                )
            orientation = "H"
    ...
    def check_driver_variables(self, name: str):
        ...
        for variable in driver_vars:
            ...
            if x is not None:
                ...
                if isinstance(driver_vars[variable], enum.EnumMeta):  # if enum
    ...
    def _set_widget_text(self, name, value):
        ...
        if hasattr(self, f"{name}_widget"):
            ...
            widget.blockSignals(True)  # block pyqtSignal indicating change since changing internally
        ...
        # Skip logging for private/internal properties (starting with _)
...
def check_if_valid(schema, item) -> bool | None:
    try:
