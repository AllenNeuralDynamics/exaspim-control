...
from pyqtgraph import GraphItem, PlotWidget, mkPen
...
class SignalChangeVar:
    """Class that emits pyqtSignal containing name when set function is used"""
    ...
    def define_waves(self, waveform: Literal["square wave", "sawtooth", "triangle wave"]) -> None:
        ...
        if "sawtooth" in waveform or "triangle" in waveform:
            if self.pos.shape[0] != 5:
                msg = f"Waveform {waveform} must have 5 points in data set. Waveform has {self.pos.shape[0]}"
                raise Exception(msg)

        elif "square" in waveform and self.pos.shape[0] != 6:
            msg = f"Waveform {waveform} must have 6 points in data set. Waveform has {self.pos.shape[0]}"
            raise Exception(msg)
        # block signals
