...
from view.widgets.miscellaneous_widgets.q_non_scrollable_tree_widget import (
    QNonScrollableTreeWidget,
)
...
class NIWidget(BaseDeviceWidget):
    def __init__(self, daq, exposed_branches: dict | None = None, advanced_user: bool = True):
        """
        ...
        if advanced_user:
            ...
            self.tree.setItemWidget(
                graph_child,
                1,
                create_widget("H", self.waveform_widget.legend, self.waveform_widget),
            )
    ...
    def update_waveform(self, channel_name: str) -> None:
        ...
        if waveform == "square wave":
            ...
            y = [
                voltages[0],
                voltages[pre_rise_point],
                voltages[max_point],
                voltages[post_rise_point],
                voltages[-1],
            ]
            ...
            if widget := getattr(self, f"{id}_widget", False):
                ...
                if "channel" in name:
                    ...
                    widget = create_widget(
                        "H",
                        getattr(self, f"{id}_widget"),
                        getattr(self, f"{id}_slider"),
                    )
                elif "timing" in name:
