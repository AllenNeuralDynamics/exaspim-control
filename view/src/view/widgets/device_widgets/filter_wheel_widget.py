...
from PyQt6.QtWidgets import QComboBox, QGraphicsEllipseItem
...
class FilterWheelWidget(BaseDeviceWidget):
    def __init__(self, filter_wheel, colors: dict | None = None, advanced_user: bool = True):
        """
        ...
        # wrap filterwheel filter property to emit pyqtSignal when set
        filter_setter = getattr(type(filter_wheel).filter, "fset")
        filter_getter = getattr(type(filter_wheel).filter, "fget")
        setattr(
            type(filter_wheel),
            "filter",
            property(filter_getter, self.filter_change_wrapper(filter_setter)),
        )
    ...
    def filter_change_wrapper(self, func: Callable) -> Callable:
        """
        Wrapper function that emits a pyqtSignal when filterwheel filter setter has been called
...
class FilterItem(ScatterPlotItem):
    """ScatterPlotItem that will emit pyqtSignal when pressed"""
    ...
    def mousePressEvent(self, ev) -> None:
        """
        Emit pyqtSignal containing filter_name when item is pressed
...
class FilterWheelGraph(PlotWidget):
    ...
    def __init__(self, filters: dict, colors: dict, diameter: float = 10.0, **kwargs):
        ...
        active = ScatterPlotItem(
            ...
            pxMode=False,
            symbol="t1",
            pos=[[self.diameter * cos(pi / 2), self.diameter * sin(pi / 2)]],
        )
    ...
    def move_wheel(self, name: str) -> None:
        ...
        for i, filt in enumerate(filters):
            ...
            timeline.frameChanged.connect(lambda i, pyqtSlot=self.points[filt]: self.move_point(i, pyqtSlot))
            timeline.frameChanged.connect(
                lambda i, pyqtSlot=self.points[self.filters[filt]]: self.move_point(i, pyqtSlot)
            )
    ...
    def move_point(self, angle: float, point: FilterItem | TextItem) -> None:
        ...
        """
        pos = [
            self.filter_path * cos(radians(angle)),
            self.filter_path * sin(radians(angle)),
        ]
        if isinstance(point, FilterItem):
            point.setData(pos=[pos])
        elif isinstance(point, TextItem):
            point.setPos(*pos)
...
class TimeLine(QObject):
    ...
    interval = pyqtProperty(int, fget=loopCount, fset=setLoopCount)
    ...
    interval = pyqtProperty(int, fget=interval, fset=setInterval)
