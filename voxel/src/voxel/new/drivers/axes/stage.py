...
from voxel.new.drivers.tigerhub.hub import TigerHub
from voxel.new.drivers.tigerhub.ops.scan import ScanPattern, ScanRConfig, ScanVConfig
from voxel.new.interfaces.axes import Axis, TTLStepper


class XYZStage[A: Axis]:
...
class ScannableStage[A: Axis](ABC, XYZStage[A]):
    @abstractmethod
    def new_scan_session(self, pattern: ScanPattern = ScanPattern.RASTER) -> ScanSession: ...


class TigerScanSession(ScanSession):
    def __init__(self, fast: TigerAxis, slow: TigerAxis, pattern: ScanPattern = ScanPattern.RASTER):
...
class TigerXYZStage(ScannableStage[TigerAxis]):
    def __init__(self, x: TigerAxis, y: TigerAxis, z: TigerAxis):
...
if __name__ == "__main__":
    ...
    x_axis = TigerAxis(hub=hub, uid="x-axis", axis_label="X")
    y_axis = TigerAxis(hub=hub, uid="y-axis", axis_label="Y")
    z_axis = TigerAxis(hub=hub, uid="z-axis", axis_label="Z")
