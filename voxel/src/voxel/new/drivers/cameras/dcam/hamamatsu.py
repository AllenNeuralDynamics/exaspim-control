...
from voxel.new.device import deliminated_float, describe, enumerated_int, enumerated_string
from voxel.new.drivers.cameras.dcam.sdk.dcamapi4 import DCAMCAP_TRANSFERINFO
from voxel.new.interfaces.camera import PixelFormat, SpimCamera, StreamInfo, TriggerMode, TriggerPolarity
from voxel.new.interfaces.camera.roi import ROI, ROIConstraints
from voxel.new.utils import thread_safe_singleton
...
class HamamatsuCamera(SpimCamera):
    ...
    def _get_frame_time_ms(self) -> float:
        ...
        if sensor_mode and "light sheet" in sensor_mode.lower():
            return (line_interval_us * roi.h) / 1000 + exposure_ms
        return (line_interval_us * roi.h / 2) / 1000 + exposure_ms
    ...
    def _set_roi(self, roi: ROI) -> None:
        ...
        self.log.debug(f"ROI set to x={roi.x}, y={roi.y}, w={roi.w}, h={roi.h}")
    ...
    def _get_roi_constraints(self) -> ROIConstraints:
        ...
        return ROIConstraints(
            grid_x=int(w_attr.valuestep) if w_attr.valuestep > 0 else 4,
            grid_y=int(h_attr.valuestep) if h_attr.valuestep > 0 else 4,
            min_x=int(w_attr.valuemin),
            min_y=int(h_attr.valuemin),
            max_w=self._sensor_width,
            max_h=self._sensor_height,
        )
