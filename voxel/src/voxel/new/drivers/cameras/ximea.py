...
from voxel.new.interfaces.camera import (
    PixelFormat,
    SpimCamera,
    StreamInfo,
    TriggerMode,
...
class XimeaCamera(SpimCamera):
    ...
    def _get_roi(self) -> ROI:
        ...
        return ROI(
            x=_to_int(self._camera.get_offsetX()),
            y=_to_int(self._camera.get_offsetY()),
            w=_to_int(self._camera.get_width()),
            h=_to_int(self._camera.get_height()),
        )
    ...
    def _set_roi(self, roi: ROI) -> None:
        ...
        # Set dimensions
        self._camera.set_width(roi.w)
        self._camera.set_height(roi.h)

        # Set offsets
        self._camera.set_offsetX(roi.x)
        self._camera.set_offsetY(roi.y)
    ...
    def _get_roi_constraints(self) -> ROIConstraints:
        ...
        return ROIConstraints(
            grid_x=_to_int(self._camera.get_width_increment()),
            grid_y=_to_int(self._camera.get_height_increment()),
            min_x=_to_int(self._camera.get_width_minimum()),
            min_y=_to_int(self._camera.get_height_minimum()),
