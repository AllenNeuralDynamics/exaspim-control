...
class PCOCamera(SpimCamera):
    ...
    def _get_roi(self) -> ROI:
        ...
        return ROI(
            x=roi_dict["x0"] - 1,  # PCO uses 1-based indexing
            y=roi_dict["y0"] - 1,
    ...
    def _get_roi_constraints(self) -> ROIConstraints:
        ...
        return ROIConstraints(
            grid_x=desc.get("roi hor steps", 1),
            grid_y=desc.get("roi vert steps", 1),
            min_x=desc.get("min size horz", 32),
            min_y=desc.get("min size vert", 32),
