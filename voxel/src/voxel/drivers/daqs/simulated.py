...
class SimulatedDaq(SpimDaq):
    ...
    def __init__(self, dev: str = "MockDev1", uid: str = "SimulatedDAQ") -> None:
    ...
    def __repr__(self) -> str:
        return f"SimulatedDaq(uid={self.uid!r}, device={self._dev!r})"
    ...
    def assign_pin(self, task_name: str, pin: str) -> PinInfo:
        ...
        if pin_lower not in [p.lower() for p in self._all_pins]:
            raise ValueError(f"Pin '{pin}' not available on device '{self._dev}'")
