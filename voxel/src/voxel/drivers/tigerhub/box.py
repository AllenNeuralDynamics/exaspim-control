...
from voxel.drivers.serial import SerialTransport
from voxel.drivers.tigerhub.model import ASIMode, AxisState, BoxInfo, Reply
from voxel.drivers.tigerhub.model.box_info import infer_comm_addr_from_who
from voxel.drivers.tigerhub.model.card_info import CardInfo
from voxel.drivers.tigerhub.model.models import ASIAxisInfo
from voxel.drivers.tigerhub.ops.joystick import (
    JoystickEnableOp,
...
from voxel.drivers.tigerhub.ops.motion import HaltOp, HereOp, HomeOp, IsAxisBusyOp, MoveAbsOp, MoveRelOp, WhereOp
from voxel.drivers.tigerhub.ops.params import GetParamOp, SetParamOp, TigerParam, TigerParams
from voxel.drivers.tigerhub.ops.scan import (
    ArrayOp,
    ArrayScanConfig,
    AutoHomeConfig,
    AutoHomeOp,
...
from voxel.drivers.tigerhub.ops.status import (
    GetAxisStateOp,
    GetBuildOp,
    GetCardMods,
    GetPiezoInfoOp,
...
from voxel.drivers.tigerhub.ops.step_shoot import (
    GetTTLModesOp,
    LoadBufferedMoveOp,
    ProbeTTLOutOp,
    ProbeTTLOutOp2,
...
from voxel.drivers.tigerhub.protocol.errors import ASIDecodeError
from voxel.drivers.tigerhub.protocol.parser import asi_parse
...
class TigerBox:
    ...
    def info(self, *, refresh: bool = False) -> BoxInfo:
        ...
        if issues := self._info.issues:
            for issue in issues:
                self.log.debug("BoxInfo warning: %s", issue)
        return self._info
    ...
    def set_joystick_polarity(self, axis: str, inverted: bool) -> None:
        ...
        if a and a.card_hex is not None and a.card_index is not None:
            ...
            self.log.debug(f"Cannot set joystick polarity for axis {axis}: missing card info")
