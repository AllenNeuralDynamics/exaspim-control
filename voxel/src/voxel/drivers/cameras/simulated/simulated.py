...
from voxel.device import deliminated_float, enumerated_int, enumerated_string
from voxel.device.props.deliminated import DeliminatedInt
from voxel.interfaces.camera import (
    BINNING_OPTIONS,
    PIXEL_FMT_TO_DTYPE,
    FrameRegion,
    PixelFormat,
...
from voxel.interfaces.helpers import parse_vec2d

from .frame_gen import ReferenceFrameGenerator
...
class SimulatedCamera(SpimCamera):
    ...
    def __init__(
        ...
        self._sensor_size_px = parse_vec2d(sensor_size_px, rtype=int)
    ...
    def frame_region(self) -> FrameRegion:
        ...
        # For simulated camera, frame region values are already in frame coordinates
        # (we apply binning during frame generation, not in the "hardware")
        return FrameRegion(
            x=DeliminatedInt(
