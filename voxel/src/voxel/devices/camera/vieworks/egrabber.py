...
class VieworksCamera(BaseCamera):
    ...
    def __init__(self, uid: str, serial: str) -> None:
        """Initialize the Camera instance."""
        super().__init__(uid)
        self.serial = str(serial)  # convert to string incase serial # is entered as int
        ...
        try:
            for egrabber in egrabber_list["grabbers"]:
                ...
                if grabber.remote.get("DeviceSerialNumber") == self.serial:
                    self.log.info(f"grabber found for S/N: {self.serial}")
                    self.grabber = grabber
                    self.egrabber = egrabber
                    break
        except Exception as e:
            self.log.exception(f"no grabber found for S/N: {self.serial}")
            msg = f"no grabber found for S/N: {self.serial}"
    ...
    def grab_frame(self) -> np.ndarray:
        """Retrieve a frame as a 2D np array with shape (rows, cols).

        :return: Frame as a 2D np array
        :rtype: np.ndarray
        """
        ...
        for category in categories:
            ...
            for feature in features:
                if (
                    ...
                    and feature not in {"BalanceRatioSelector", "BalanceWhiteAuto"}
                ):
    ...
    def _get_min_max_step_values(self) -> None:
        ...
        try:
            ...
            self.log.debug(f"min exposure time not available for camera {self.serial}")
        ...
        try:
            ...
            self.log.debug(f"max exposure time not available for camera {self.serial}")
        # minimum width
        try:
            ...
            self.log.debug(f"min width not available for camera {self.serial}")
        # maximum width
        try:
            ...
            self.log.debug(f"max width not available for camera {self.serial}")
        # minimum height
        try:
            ...
            self.log.debug(f"min height not available for camera {self.serial}")
        # maximum height
        try:
            ...
            self.log.debug(f"max height not available for camera {self.serial}")
        # minimum offset x
        try:
            ...
            self.log.debug(f"min offset x not available for camera {self.serial}")
        # maximum offset x
        try:
            ...
            self.log.debug(f"max offset x not available for camera {self.serial}")
        # minimum offset y
        try:
            ...
            self.log.debug(f"min offset y not available for camera {self.serial}")
        # maximum offset y
        try:
            ...
            self.log.debug(f"max offset y not available for camera {self.serial}")
        # step exposure time
        # convert from us to ms
        try:
            ...
            self.log.debug(f"step exposure time not available for camera {self.serial}")
        # step width
        try:
            ...
            self.log.debug(f"step width not available for camera {self.serial}")
        # step height
        try:
            ...
            self.log.debug(f"step height not available for camera {self.serial}")
        # step offset x
        try:
            ...
            self.log.debug(f"step offset x not available for camera {self.serial}")
        # step offset y
        try:
            ...
            self.log.debug(f"step offset y not available for camera {self.serial}")
        ...
        try:
            ...
            self.log.debug(f"min line interval is not available for camera {self.serial}")
        ...
        try:
            ...
            self.log.debug(f"max line interval is not available for camera {self.serial}")
        # step line interval
    ...
    def _query_binning(self) -> None:
        ...
        for binning in binning_options:
            ...
            except (AttributeError, KeyError, TypeError, ValueError, GenTLException):
