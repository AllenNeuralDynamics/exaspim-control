"""Binding utilities for hardware-bound widgets.

NOTE: This module is kept for potential future use but is currently unused.
The simpler getter/setter pattern in the V* widgets is preferred (YAGNI).
"""
...
class FieldBinder[ReadT, WriteT](QObject):
    ...
    value_changed = pyqtSignal(object)  # emits read value (e.g., DeliminatedFloat)
    command_sent = pyqtSignal(object)  # emits written value
    ...
    value = pyqtProperty(object, fget=get_value, fset=set_value)
...
class ValueBinding[ReadT, WriteT](QObject):
    ...
    value_changed = pyqtSignal(object)  # When display value changes
    command_sent = pyqtSignal(object)  # When command sent to hardware
...
class ValidatedValueBinding[T: int | float](QObject):
    ...
    value_changed = pyqtSignal(object)  # When display value changes
    command_sent = pyqtSignal(object)  # When command sent to hardware
    verification_completed = pyqtSignal(bool)  # True if verified, False if mismatch
