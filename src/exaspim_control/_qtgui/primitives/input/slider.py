...
class LockableSlider(QWidget):
    ...
    Usage:
        track = VLockableSlider(min_value=0, max_value=100, color="#0078d4")
        track.inputReleased.connect(on_user_input)
        adapter.propertyUpdated.connect(route_to_track)

        # In property router:
        track.setActual(current_value)
        track.setTarget(target_value)
    """

    inputReleased = pyqtSignal(float)  # Emitted when user releases input slider
    ...
    def __init__(
        ...
        """Initialize the lockable slider.

        :param min_value: Minimum value in the range
        :param max_value: Maximum value in the range
        :param color: Color for progress bar and target indicator
