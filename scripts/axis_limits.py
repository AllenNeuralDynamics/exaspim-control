...
# PORT_NAME = "COM6"  # a string indicating the port name.
...
# print(lower_stage_limits)
# print("Previous upper stage limits:")
# print(upper_stage_limits)
...
# for x in axes:
#     input(f"Use the joystick to move to the LOWER travel limit of {x.upper()}. Press enter when ready.")
#     axis_pos_dict = box.get_position(x)
#     axis_pos_dict_mm = {x: v / STEPS_PER_UM / 1000.0 for x, v in axis_pos_dict.items()}
#     box.set_lower_travel_limit(**axis_pos_dict_mm)
#     input(f"Use the joystick to move to the UPPER travel limit of {x.upper()}. Press enter when ready.")
#     axis_pos_dict = box.get_position(x)
#     axis_pos_dict_mm = {x: v / STEPS_PER_UM / 1000.0 for x, v in axis_pos_dict.items()}
