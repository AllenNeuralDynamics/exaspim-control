...
class MetaDataLaunchTests(unittest.TestCase):
    ...
    def test_writing(self):
        ...
        instrument_config = {
            "instrument": {"channels": {"CH639": {"filters": ["BP639"], "lasers": ["639 nm"], "cameras": ["vp-151mx"]}}}
        }
        ...
        metadata_launch = MetadataLaunch(
        ...
        expected_schema = acquisition.Acquisition(
            experimenter_full_name=["Chris P. Bacon"],
            specimen_id="123",
            subject_id="123",
            instrument_id="exaspim123",
            ...
            chamber_immersion={"medium": "oil", "refractive_index": 1.33},
            axes=[
                {"name": "X", "dimension": 2, "direction": "Anterior_to_posterior"},
                {"name": "Y", "dimension": 1, "direction": "Inferior_to_superior"},
                {"name": "Z", "dimension": 0, "direction": "Left_to_right"},
            ],
            tiles=[
                {
                    "file_name": "tile_000000_ch_CH639_camera_vp-151mx",
    ...
    def test_saving(self):
        ...
        mocked_transfer.configure_mock(external_path=RESOURCES_DIR / "external_test", acquisition_name="test")

        mocked_transfer1 = MagicMock()
        mocked_transfer1.configure_mock(external_path=RESOURCES_DIR / "external_test1", acquisition_name="test")

        mocked_writer = MagicMock()
        mocked_writer.configure_mock(local_path=RESOURCES_DIR / "local_test", acquisition_name="test")

        mocked_writer1 = MagicMock()
        mocked_writer1.configure_mock(local_path=RESOURCES_DIR / "local_test1", acquisition_name="test")
        try:
            # test one transfer
            mocked_acquisition = MagicMock()
            mocked_acquisition.configure_mock(
                metadata=mocked_metadata, config=acquisition_config, transfers={"vp-151mx": {"test": mocked_transfer}}
            )
            ...
            assert os.path.exists(RESOURCES_DIR / "external_test/test/exaspim_acquisition.json")
            ...
            mocked_acquisition.configure_mock(
                ...
                transfers={"vp-151mx": {"test": mocked_transfer, "test1": mocked_transfer1}},
            )
            ...
            metadata_launch.create_acquisition_json()
            assert os.path.exists(RESOURCES_DIR / "external_test/test/exaspim_acquisition.json")
            assert os.path.exists(RESOURCES_DIR / "external_test1/test/exaspim_acquisition.json")
            ...
            metadata_launch.create_acquisition_json()
            assert os.path.exists(RESOURCES_DIR / "local_test/test/exaspim_acquisition.json")
            ...
            metadata_launch.create_acquisition_json()
            assert os.path.exists(RESOURCES_DIR / "local_test/test/exaspim_acquisition.json")
            assert os.path.exists(RESOURCES_DIR / "local_test1/test/exaspim_acquisition.json")
